...
internals.Connection.prototype.set = function (key, value, ttl, callback) {
    ...
    if(this.settings.buffer){
        if (Buffer.isBuffer(value)) {
            ...
            return callback(new Error('Value is not a Buffer'));
        }
    } else {
        try {
    ...
    if (this.settings.maxByteSize) {
        envelope.byteSize = 53 + (this.settings.buffer ? envelope.item.size : Buffer.byteLength(envelope.item)) + Buffer.byteLength(key.segment) + Buffer.byteLength(key.id);     // Envelope size without value: 53 bytes
